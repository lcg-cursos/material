...
El alumno debe ser capaz de aplicar los filtros con grep y conocer las expresiones regulares.
---
...
|Opción |   Comportamiento |
|----    |---                 |
|--color | Marca el texto que corresponde al patrón |
|-E |Interpreta el patr´on como una expresi´on regular extendida tipo POSIX|
|-P |Interpreta el patr´on como una expresi´on regular extendida, tipo Perl|
|-f |Lee uno o más patrones a partir de un archivo|
|-i |Ignora mayúsculas/minúsculas|
|-n |Imprime el número de línea donde se encontró el patrón|
|-v |Selecciona las l´ıneas en donde no ocurre el patrón|
...
- Piensa en los pasos necesarios para obtener la respuesta que buscas antes de ejecutar tu primer comando

- Documenta tus códigos describiendo la funcionalidad de cada sección

- Analiza el output para encontrar fallas lógicas o comportamientos inesperados

- Recuerda que siempre puedes acceder al manual de un comando o buscar ayuda en internet para entender su funcionamiento
...
- Copia los archivos ubicados en /home/lgomez/WelcomeBioinfo/datos/practica5 a tu carpeta data
...
- Los archivos llamados cholerae son anotaciones de la bacteria Vibrio cholerae

- Los archivos llamados anthracis son anotaciones de la bacteria Bacillus anthracis
...
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/thumb/9/9d/Cholera_bacteria_SEM.jpg/440px-Cholera_bacteria_SEM.jpg")
```
]
.pull-right[
...
```{r, out.width = "300px",fig.align='center', fig.cap=''}
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/Bacillus_anthracis.png/654px-Bacillus_anthracis.png")
```
]
...
- Obtener los valores únicos y el número de repeticiones para cada uno
de ellos
...
Para ambos organismos la fuente de casi la mitad de las anotaciones es la
inferencia de homolog´ıa para los genes o proteínas
...
- Obtener el nombre de los genes compartidos en B. anthracis
...
```bash
## Obtener los genes unicos de V. cholerae
grep -v "#" data/cholerae.gff | grep -P "RefSeq\tgene" | cut -f9 | \
cut -d ';' -f2 | sort -u > cholerae_genes.txt

## Contar el numero de genes unicos de V. cholerae
wc -l cholerae_genes.txt

## Buscarlos en B. anthracis
grep -f cholerae_genes.txt data/anthracis.gff \
> cholerae_anthracis_genes.gff

## Obtener numero de genes unicos compartidos
...
- 523 también existen en B. anthracis
]

---
## Ejercicio 3 Pregunta

**¿Cuántos de las proteínas de Vibrio cholerae también existen en Bacillus anthracis?**

.content-box-blue[

Para fines de este ejercicio, una prote´ına ser´a considerada la misma si tiene
...
```bash
## Buscarlos en B. anthracis
grep -iFf cholerae_products_uniq.txt data/anthracis.gff \
...
- 1030 también existen en B. anthracis
]

---
## Ejercicio 4 Pregunta

**¿Cuál es la longitud aproximada de los genomas de Vibrio cholerae y de
Bacillus anthracis? ¿Cuántos cromosomas tiene cada bacteria?**
...
- Obtener las líneas encabezado del fasta genómico y contarlas

- Excluir las líneas encabezado del fasta genómico y contar los caracteres
...
V. cholerae:
  - 2 cromosomas
  - aprox 4083883 pares de bases en su genoma
...
**¿Cuál de las siguientes bacterias: Escherichia coli, Vibrio cholerae y
Bacillus anthracis, tiene: **
...
- Después ordenar numéricamente por posición de inicio
...
```bash
## Ordenar por feature_type y posicion V. cholerae
sort -k3,3 -k4,4n data/cholerae.gff | less -S

## Ordenar por feature_type y posicion B. anthracis
...
- Obtener los nombre de los genes asociados y contarlos
...
El archivo FASTA con las secuencias proteicas de un organismo específico (el cual descargaste de NCBI) tiene secuencias que han sido asigndas a distintos niveles taxon´omicos, esta información se encuentra en el header de cada secuencia entre corchetes, ¿cuáles son los distintos niveles
...
```bash
## Niveles taxonomicos proteinas E. coli
grep ">" data/coli_protein.fna | rev | cut -d '[' -f 1 | rev | \
cut -d ']' -f1 | sort -u

## Niveles taxonomicos proteinas V. cholerae
grep ">" data/cholerae_protein.fna | rev | cut -d '[' -f 1 | rev | \
cut -d ']' -f1 | sort -u

## Niveles taxonomicos proteinas B. anthracis
...
**¿Existe alguna proteína en los archivos FASTA de secuencias proteícas que no se encuentren en el gff de la anotación para *Escherichia coli*, *Vibrio cholerae* y *Bacillus anthracis* ?**
...
```bash
## Obtener los ID de las prote´ınas E. coli
...
## Obtener los ID de la anotacion E. coli
cut -f9 data/coli.gff | cut -d ';' -f1 | sort -u > coli_id_fromgff.txt

## Obtener la interseccion de ambas listas
...
```bash
## Obtener los ID de las proteinas V. cholerae
...
## Obtener los ID de la anotacion V. cholerae
...
```bash
## Obtener los ID de las proteinas B. anthracis
...
## Obtener los ID de la anotacion B. anthracis
...
E.coli:
- 4242 protein id del archivo FASTA de proteinas
...
- Es una secuencia de caracteres que definen un patr´on de búsqueda
...
¿Cómo definirías una secuencia de patrones que describa a todas las siguientes palabras?
...
| pipe      | Identifica la expresiòn regular antes y después el símbolo |
...
---
## ¿Cómo aplicamos una expresión regular?

Recupera las líneas de la anotación de E. coli que describen los productos asociados a nuestros IDs utilizando expresiones regulares


```bash
## Pidiendo ayuda
## grep se utiliza para buscar patrones
man grep
```
...
Recupera las líneas de la anotación de E. coli que describen los productos
asociados a nuestros IDs utilizando expresiones regulares

```bash
## Aplicando nuestra expresion regular
## a la anotacion de E. coli
grep -P 'YP_5884\d{2}\.1' data/coli.gff | less -S
```

¿Recuperamos únicamente las líneas que pertenecen a nuestros ID problema?

---
## Aplicando expresiones regulares

¿Cuántos IDs diferentes recupera nuestra expresión regular?

```bash
## IDs recuperados (36)
grep -P 'YP_5884\d{2}\.1' data/coli.gff | \
cut -f9 | cut -d';' -f1 | sort -u | wc -l
## IDs iniciales (7)
```

Algunos de los IDs adicionales

- YP 588435.1
- YP 588463.1
- YP 588478.1

¿Cómo podemos disminuir el número de falsos positivos (IDs adicionales)?

---
## Refinando una expresión regular
...
Nuestra expresión regular original

```
YP 5884\d{2}\.1
```

Refinando la expresi´on regular
...
```bash
## IDs recuperados (18)
grep -P 'YP_5884[6|7]\d\.1' data/coli.gff | \
cut -f9 | cut -d';' -f1 | sort -u
## IDs iniciales (7)
```

Aún no es perfecta, pero funciona mejor

---
## Practicando con expresiones regulares

Explora el archivo de trabajo: regexp example.txt

Ubicado en tu carpeta data


---
## Aplicando expresiones regulares

1. Recupera manzana y melon
2. Recupera manzana y banana
3. Recupera melon y peron
4. Recupera lampara y carta
5. Recupera manzana, banana, lampara y carta
6. Recupera peron y peloton
7. Recupera melon, peron y peloton

No existe una UNICA solución correcta

En cada caso se deben recuperar únicamente las palabras solicitadas

---
## Ejercicio 1. Recupera manzana y melon

Recupera manzana y melon

```bash
## Empiezan con m
grep -P '^m' data/regexp_example.txt
```

---
## Ejercicio 2. Recupera manzana y banana

Recupera manzana y banana

```bash
## Terminan con ana
grep -P 'ana$' data/regexp_example.txt
```

---
## Ejercicio 3. Recupera melon y peron

Recupera melon y peron

```bash
## Existe una letra cualquiera al inicio de la palabra
## Seguida de una e
## Seguida de unicamente una letra
## Seguida de on
## Terminan con on
grep -P '^\we\won$' data/regexp_example.txt
```

---
## Ejercicio 4. Recupera manzana, lampara y carta


Recupera manzana, lampara y carta

```bash
## Existe una a
## Seguida de dos letras,
## Seguida de otra a
grep -P 'a\w{2}a' data/regexp_example.txt
```

---
## Ejercicio 5. Recupera manzana, banana, lampara y carta


Recupera manzana, banana, lampara y carta

```bash
## Terminan con a
grep -P 'a$' data/regexp_example.txt
```

---
## Ejercicio 6. Recupera peron y peloton

Recupera peron y peloton

```bash
## Inician con pe
## Seguido de una o m´as letras
## Seguido de on
grep -P '^pe\w+on' data/regexp_example.txt
```

---
## Ejercicio 7. Recupera melon, peron y peloton

```bash
## A partir del inicio de la palabra existe una letra
## Seguido de una e
## Seguido de una o m´as letras
## Seguido de on
grep -P '^\we\w+on' data/regexp_example.txt
```
